...
`);
    ...
    //获取用户id
    const userId=document.querySelectorAll('#headerNeue2 .idBadgerNeue a.avatar')[0].href.split('/user/')[1];
    ...
    function Process(){
        itemsList.forEach( (elem, index) => {
            ...
            if(User===userId && User_Point)
                localStorage.setItem(userId+'Point'+ID,User_Point);
            // 第二次应该进这里面
    ...
    function DisplayStatus(ID,index,info){
        ...
        let Point_My = localStorage.getItem(userId+'Point'+ID);
